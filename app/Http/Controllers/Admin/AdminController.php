...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $jobs['total_jobs'] = Job::where('status',1)->count();
        $jobs['my_jobs'] = Job::where('status',1)->where('owner_id',1)->where('owner_type',1)->count();
        $jobs['finished_jobs'] = Job::where('status',1)->where('job_status',2)->count();
        $jobs['running_jobs'] = Job::where('status',1)->where('job_status',1)->count();
        $jobs['pending_jobs'] = Job::where('status',1)->where('job_status',0)->count();
        $jobs['canceled_jobs'] = Job::where('status',0)->count();
        ...
        $newTickets = SupportTicket::with('user')->orderBy('created_at', 'desc')->whereStatus(0)->limit(9)->get();
        return view('admin.dashboard', compact('pageTitle', 'widget', 'withdrawalsChart', 'depositsChart', 'deposit', 'withdrawals', 'userLogins', 'newTickets','jobs'));
    }
